...
import { Clock, Calendar, Zap, ShieldCheck, MapPin, Navigation, Compass, Filter, ArrowUpRight } from 'lucide-react'
  ...
  return (
    <motion.div
      ...
      className={`relative p-5 sm:p-6 rounded-2xl border transition-all duration-300 cursor-pointer group ${
        isSelected
          ? 'bg-[#0E1A11] border-[#7ED321] shadow-[0_0_25px_rgba(126,211,33,0.3)] translate-x-1'
          : 'bg-[#0D140E]/90 border-[#7ED321]/20 hover:border-[#7ED321]/60 hover:bg-[#0E1710]'
      }`}
    >
      {/* Top Gloss Line */}
      <div
        className={`absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl transition-opacity duration-300 ${
          isSelected
            ? 'bg-gradient-to-r from-[#7ED321] via-[#3DD9FF] to-[#7ED321] opacity-100'
            : 'bg-gradient-to-r from-transparent via-[#7ED321]/30 to-transparent opacity-0 group-hover:opacity-100'
        }`}
      />

      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        {/* Time Badge */}
        <div className="flex items-center gap-1.5 font-mono-data text-xs text-[#7ED321] font-bold tabular-nums">
          <Clock size={13} className="text-[#7ED321]" />
        ...
        <span
          className={`font-mono-data text-[10px] uppercase font-bold px-2.5 py-0.5 rounded border transition-colors ${
            isSelected
              ? 'bg-[#7ED321] text-[#070B08] border-[#7ED321]'
              : 'bg-[#7ED321]/10 text-[#7ED321] border-[#7ED321]/30'
          }`}
        >
          {event.type}
        </span>
      ...
      <h3
        className={`font-display text-xl sm:text-2xl mb-2 transition-colors ${
          isSelected ? 'text-[#7ED321]' : 'text-white group-hover:text-[#7ED321]'
        }`}
      >
        {event.title}
      </h3>
      ...
      <div className="mt-3 pt-3 border-t border-[#7ED321]/15 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 font-mono-data text-xs text-[#8A9488]">
          <MapPin size={13} className="text-[#7ED321] shrink-0" />
        ...
        <button
          ...
          className={`inline-flex items-center gap-1 font-mono-data text-[11px] uppercase font-bold px-2.5 py-1 rounded-lg transition-all ${
            isSelected
              ? 'bg-[#7ED321] text-[#070B08] shadow-[0_0_10px_rgba(126,211,33,0.4)]'
              : 'bg-[#070B08] text-[#7ED321] border border-[#7ED321]/30 hover:bg-[#7ED321]/20'
          }`}
        >
          <Navigation size={11} className={isSelected ? 'animate-bounce' : ''} />
          <span>{isSelected ? 'Route Active' : 'View Path'}</span>
          <span className="text-[10px] opacity-75">({event.walkTime})</span>
        </button>
      </div>
    </motion.div>
  )
...
export default function Timeline() {
  ...
  const handleSelectEvent = (event: ScheduleEvent | null) => {
    ...
    // Toggle logic: if already selected, reset to all routes
    if (selectedEventId === event.id) {
      setSelectedEventId(null)
    } else {
      setSelectedEventId(event.id)
      // Scroll event card into view if needed
      document
  ...
  return (
    <section
      id="schedule"
      className="py-20 lg:py-32 relative bg-[#070B08] border-t border-b border-[#7ED321]/15 overflow-hidden"
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#7ED321]/40 to-transparent pointer-events-none" />
      ...
      <div className="section-container relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-10 pb-6 border-b border-[#7ED321]/15">
          {/* Day 1 / Day 2 Tabs */}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: `1px solid ${isActive ? '#7ED321' : 'rgba(126,211,33,0.25)'}`,
                    boxShadow: isActive ? '0 0 20px rgba(126,211,33,0.35)' : 'none',
                  }}
                >
                  <Calendar size={14} />
                  <span>
          ...
          <div className="flex items-center gap-2 overflow-x-auto max-w-full pb-2 sm:pb-0">
            <Filter size={13} className="text-[#7ED321] shrink-0 hidden md:block" />
            {['all', 'keynote', 'panel', 'competition', 'hackathon', 'expo'].map((type) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl font-mono-data text-[11px] uppercase tracking-wider whitespace-nowrap transition-all ${
                  typeFilter === type
                    ? 'bg-[#7ED321]/20 text-[#7ED321] border border-[#7ED321] font-bold'
                    : 'bg-[#0D140E] text-[#8A9488] border border-[#7ED321]/15 hover:text-white'
                }`}
              >
                {type}
              </button>
        ...
        {/* Mobile View Toggle Pills (Visible only on small screens) */}
        <div className="lg:hidden flex items-center justify-center p-1 rounded-2xl bg-[#0D140E] border border-[#7ED321]/20 mb-6">
          <button
            onClick={() => setMobileView('both')}
            className={`flex-1 py-2 font-mono-data text-xs uppercase font-bold rounded-xl transition-all ${
              mobileView === 'both' ? 'bg-[#7ED321] text-[#070B08]' : 'text-[#8A9488]'
            }`}
          >
            Split View
          </button>
          <button
            ...
            className={`flex-1 py-2 font-mono-data text-xs uppercase font-bold rounded-xl transition-all ${
              mobileView === 'map' ? 'bg-[#7ED321] text-[#070B08]' : 'text-[#8A9488]'
            }`}
          >
            2D Map
          </button>
          <button
            ...
            className={`flex-1 py-2 font-mono-data text-xs uppercase font-bold rounded-xl transition-all ${
              mobileView === 'list' ? 'bg-[#7ED321] text-[#070B08]' : 'text-[#8A9488]'
            }`}
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* Right Column: Events List Stream */}
          <div
          ...
          >
            ...
            <div className="flex items-center justify-between px-2 mb-2">
              <span className="font-mono-data text-xs uppercase text-[#8A9488]">
                Select an event to preview route on map
              </span>
              <span className="font-mono-data text-xs text-[#7ED321] font-bold">
