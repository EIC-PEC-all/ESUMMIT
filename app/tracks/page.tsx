...
import { Zap, Users, Store, Code2, Network, ArrowLeft, ArrowRight, CheckCircle2, Trophy, Clock, X, Sparkles } from 'lucide-react'
import Nav from '@/components/Nav'
import Footer from '@/components/Footer'
import Concierge from '@/components/Concierge'
import { TRACKS, TRACKS as ALL_TRACKS } from '@/lib/data'
      ...
      <section className="relative pt-36 pb-20 border-b border-[#8A90A6]/10 overflow-hidden">
        ...
        <div className="section-container relative z-10">
          ...
          <div className="max-w-3xl">
            ...
            >
              SUMMIT TRACKS &amp; <br />
              <span style={{ color: 'var(--accent-ignite)' }}>COMPETITIONS</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-3">
              {['All', 'Competitions', 'Exhibitions', 'Sessions', 'Networking'].map((cat) => (
                <button
                  ...
                  style={{
                    background: activeCategory === cat ? 'var(--accent-ignite)' : 'rgba(138,144,166,0.08)',
                    color: activeCategory === cat ? '#F5F3EE' : 'var(--text-muted)',
                    border: `1px solid ${activeCategory === cat ? 'transparent' : 'rgba(138,144,166,0.12)'}`,
                  }}
                >
                  {cat}
                </button>
            ...
            {filteredTracks.map((track) => {
              ...
              return (
                <motion.div
                  ...
                  className="rounded-2xl p-8 flex flex-col justify-between border border-[#8A90A6]/12 bg-panel hover:border-ignite/40 transition-all duration-300 group"
                >
                  <div>
                    <div className="flex items-center justify-between mb-6">
                      ...
                      >
                        <IconComp size={24} style={{ color: track.color }} />
                      </div>
                      <span
                        className="font-mono-data text-[10px] uppercase tracking-widest px-3 py-1 rounded-full"
                        style={{ background: 'rgba(138,144,166,0.08)', color: 'var(--text-muted)' }}
                      >
                        {track.category}
                      </span>
                    </div>

                    <h3 className="font-display text-3xl mb-2 text-primary group-hover:text-ignite transition-colors">
                      {track.title}
                    </h3>
                    <p className="font-body text-sm text-muted mb-6">{track.tagline}</p>

                    <div className="space-y-2 mb-8 py-4 border-t border-b border-[#8A90A6]/10 font-mono-data text-xs">
                      <div className="flex justify-between">
                        <span className="text-muted">Prize / Value:</span>
                        <span className="text-signal font-semibold">{track.prize}</span>
                      </div>
                      <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-ignite">{track.deadline}</span>
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full py-3.5 rounded-xl font-body font-semibold text-sm btn-ghost justify-center hover:border-ignite hover:text-ignite"
                  >
        ...
        {selectedTrack && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-void/80 backdrop-blur-md">
            <motion.div
              ...
              className="w-full max-w-xl rounded-2xl p-8 bg-panel border border-[#8A90A6]/20 shadow-2xl relative max-h-[90vh] overflow-y-auto"
            >
              <button
              ...
              {!registered ? (
                <>
                  ...
                  <div className="space-y-4 mb-6">
                    <div className="p-4 rounded-xl bg-void border border-[#8A90A6]/10">
                      <h4 className="font-mono-data text-xs uppercase text-ignite mb-2">Key Highlights</h4>
                    ...
                    <div className="p-4 rounded-xl bg-void border border-[#8A90A6]/10">
                      <h4 className="font-mono-data text-xs uppercase text-signal mb-1">Eligibility Criteria</h4>
                ...
                <div className="py-8 text-center">
                  ...
                  <button onClick={() => setSelectedTrack(null)} className="btn-ignite px-8 py-3">
                    Close Window
                  </button>
                </div>
              )}
